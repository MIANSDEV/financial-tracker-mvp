...
export default function SubscriptionsPage() {
  ...
  const filterLabels: Record<string, string> = {
    all: t.subscriptions.all,
    paid: t.subscriptions.paid,
    pending: t.subscriptions.pending,
    overdue: t.subscriptions.overdue,
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">{t.subscriptions.title}</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{t.subscriptions.trackDesc}</p>
        </div>
        <Button onClick={() => setModalOpen(true)} leftIcon={<Plus className="w-4 h-4" />}>
          {t.subscriptions.addPaymentRecord}
        </Button>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: t.subscriptions.totalCollected, value: formatCurrency(totalCollected), icon: <DollarSign className="w-5 h-5" />, color: 'text-green-600 bg-green-50 dark:bg-green-900/20 dark:text-green-400' },
          { label: t.subscriptions.totalPending, value: formatCurrency(totalPending), icon: <Clock className="w-5 h-5" />, color: 'text-yellow-600 bg-yellow-50 dark:bg-yellow-900/20 dark:text-yellow-400' },
          { label: t.subscriptions.totalOverdue, value: formatCurrency(totalOverdue), icon: <AlertTriangle className="w-5 h-5" />, color: 'text-red-600 bg-red-50 dark:bg-red-900/20 dark:text-red-400' },
          { label: t.subscriptions.activeCompanies, value: String(activeCount), icon: <CheckCircle className="w-5 h-5" />, color: 'text-brand-600 bg-brand-50 dark:bg-brand-900/20 dark:text-brand-400' },
        ].map((stat) => (
          <Card key={stat.label}>
      ...
      <div className="flex gap-2">
        {(['all', 'pending', 'overdue', 'paid'] as const).map((s) => (
          ...
          >
            {s === 'all'
              ? `${filterLabels.all} (${payments.length})`
              : `${filterLabels[s]} (${payments.filter((p) => p.status === s).length})`}
          </button>
        ))}
      </div>
      ...
      <Card padding={false}>
        {loading ? (
          ...
          <div className="flex flex-col items-center py-16 text-gray-400 dark:text-gray-600">
            ...
            <p className="text-sm font-medium">{t.subscriptions.noPaymentRecords}</p>
            <p className="text-xs mt-1">{t.subscriptions.noPaymentDesc}</p>
          </div>
        ) : (
              ...
              <thead>
                <tr className="border-b border-gray-100 dark:border-gray-800">
                  {[
                    ...
                    t.subscriptions.dueDate,
                    t.subscriptions.paidOn,
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1.5">
                        ...
                        <Badge variant={statusVariant[payment.status]} className="capitalize">
                          {filterLabels[payment.status] ?? payment.status}
                        </Badge>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      {payment.status !== 'paid' && (
                        <div className="flex gap-2">
                          ...
                          >
                            {markingPaid === payment.id ? '...' : t.subscriptions.markPaid}
                          </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-800">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">{t.subscriptions.addPaymentRecord}</h2>
            ...
            <form onSubmit={handleSubmit(handleCreate)} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t.subscriptions.company}</label>
                <select
                  {...register('companyId')}
                ...
                >
                  <option value="">{t.subscriptions.selectCompany}</option>
                  {companies.map((c) => (
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t.subscriptions.plan}</label>
                <select
                  {...register('plan')}
                  className="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 text-sm bg-white dark:bg-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500"
                >
                  {Object.entries(SUBSCRIPTION_PLANS).filter(([, v]) => v.price > 0).map(([key, val]) => (
                    <option key={key} value={key}>{val.label} — {formatCurrency(val.price)}{t.common.perMonth}</option>
                  ))}
                </select>
              </div>

              <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <span className="text-sm text-gray-600 dark:text-gray-400">{t.subscriptions.amountDue}</span>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t.subscriptions.dueDate}</label>
                <input
                  {...register('dueDate')}
                  type="date"
                  className="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 text-sm bg-white dark:bg-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t.subscriptions.notes}</label>
                <input
                  {...register('notes')}
                  type="text"
              ...
              <div className="flex gap-3 pt-2">
                ...
                <Button type="submit" loading={saving} className="flex-1">{t.subscriptions.createRecord}</Button>
              </div>
            </form>
          </div>
        </div>
